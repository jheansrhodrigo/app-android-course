...
public class AulaEntidadeKotlin implements Serializable{
    private String nome;
    ...
    public static void insere(Activity activity, AulaEntidadeKotlin aula){
        ...
        db.execSQL("insert into aula_kotlin (nome, descricao, chave_url, concluido, pergunta, resposta) values(?,?,?,?,?,?)", arrayParam);
    }

    public static void concluirAula(Activity activity, AulaEntidadeKotlin aula){
        Log.i("Concluir ", aula.getNome());
        SQLiteDatabase db = DAOSQLiteAula.getDBInstance(activity);

        db.execSQL("update aula_kotlin set concluido = 1 where nome = '"+aula.getNome()+"'");
    }

    public static void limparAulas(Activity activity){
        SQLiteDatabase db = DAOSQLiteAula.getDBInstance(activity);

        db.execSQL("delete from aula_kotlin");
    }

    public static void excuirAula(Activity activity, String nome){
        SQLiteDatabase db = DAOSQLiteAula.getDBInstance(activity);

        db.execSQL("delete from aula_kotlin where nome = '" + nome + "'");
    }

    public static List<AulaEntidadeKotlin> buscaTodos(Activity activity, String filter){
        String sqlWhere = "";
        ...
        ArrayList<AulaEntidadeKotlin> lista= new ArrayList<>();
        Cursor cursor= DAOSQLiteAula.getDBInstance(activity).rawQuery("select " +
                ...
                " pergunta," +
                " resposta" +
                " from aula_kotlin "+ sqlWhere +" order by nome",null);

        while(cursor.moveToNext()) {
            AulaEntidadeKotlin aula = new AulaEntidadeKotlin();
